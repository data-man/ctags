...
*
*   This module contains functions for generating tags for Nim language
*   files.
*/
...
static const char *const NimAccesses[COUNT_ACCESS] = {
	"private",
	"protected",
	"public"
};
...
static fieldDefinition NimFields[COUNT_FIELD] = {
	{ .name = "decorators",
...
static roleDesc NimModuleRoles [] = {
	{ true, "imported",
	  "imported modules" },
	{ true, "namespace",
...
static kindDefinition NimKinds[COUNT_KIND] = {
	...
	{true, 'i', "module",    "modules",
	 .referenceOnly = true,  ATTACH_ROLES(NimModuleRoles)},
	{true, 'x', "unknown",   "name referring a class/variable/function/module defined in other module",
	 .referenceOnly = false, ATTACH_ROLES(NimUnknownRoles)},
	{false, 'z', "parameter", "function parameters" },
	{false, 'l', "local",    "local variables" },
};

static const keywordTable NimKeywordTable[] = {
	/* keyword			keyword ID */
	{ "as",				KEYWORD_as				},
	{ "async",			KEYWORD_async			},
	{ "cdef",			KEYWORD_cdef			},
...
struct nimNestingLevelUserData {
	int indentation;
};
#define PY_NL(nl) ((struct nimNestingLevelUserData *) nestingLevelGetUserData (nl))

static langType Lang_nim;
static unsigned int TokenContinuationDepth = 0;
static tokenInfo *NextToken = NULL;
static NestingLevels *NimNestingLevels = NULL;
static objPool *TokenPool = NULL;


/* follows PEP-8, and always reports single-underscores as protected
 * See:
 * - http://www.nim.org/dev/peps/pep-0008/#method-names-and-instance-variables
 * - http://www.nim.org/dev/peps/pep-0008/#designing-for-inheritance
 */
static accessType accessFromIdentifier (const vString *const ident,
                                        nimKind kind, int parentKind)
{
...
static void initNimEntry (tagEntryInfo *const e, const tokenInfo *const token,
                             const nimKind kind)
{
	accessType access;
	int parentKind = -1;
	...
	nl = nestingLevelsGetCurrent (NimNestingLevels);
	if (nl)
	{
	...
	e->extensionFields.access = NimAccesses[access];
...
static int makeFunctionTag (const tokenInfo *const token,
                            ...
                            const vString *const decorators)
{
	if (NimKinds[K_FUNCTION].enabled)
	{
		tagEntryInfo e;

		initNimEntry (&e, token, K_FUNCTION);
...
static int makeSimpleNimTag (const tokenInfo *const token, nimKind const kind)
{
	if (NimKinds[kind].enabled)
	{
		tagEntryInfo e;

		initNimEntry (&e, token, kind);
		return makeTagEntry (&e);
	}
...
static int makeSimpleNimRefTag (const tokenInfo *const token,
                                   const vString *const altName,
                                   nimKind const kind,
                                   int roleIndex, xtagType xtag)
{
	if (isXtagEnabled (XTAG_REFERENCE_TAGS) &&
	    NimKinds[kind].roles[roleIndex].enabled)
	{
		tagEntryInfo e;
...
getNextChar:
	...
	switch (c)
	{
		...
		default:
			...
			else
			{
				...
				token->keyword = lookupKeyword (vStringValue (token->string), Lang_nim);
	 ...
	 * https://docs.nim.org/3.6/reference/lexical_analysis.html#implicit-line-joining */
	if (token->type == '(' ||
	    token->type == '{' ||
	    token->type == '[')
	{
...
static bool readCDefName (tokenInfo *const token, nimKind *kind)
{
	readToken (token);
                                ...
                                nimKind kind, bool isCDef)
{
	...
	lv = nestingLevelsPush (NimNestingLevels, corkIndex);
	...
	if (parameterCount > 0)
	{
		...
		{
			makeSimpleNimTag (parameterTokens[i], K_PARAMETER);
...
static bool parseImport (tokenInfo *const token)
{
	...
	if (token->keyword == KEYWORD_import)
	{
		...
		if (fromModule)
		{
			...
			makeSimpleNimRefTag (fromModule, NULL, K_MODULE,
			                        PYTHON_MODULE_NAMESPACE,
			                        XTAG_UNKNOWN);
		}
				...
				if (token->keyword == KEYWORD_as)
				{
					...
					if (token->type == TOKEN_IDENTIFIER)
					{
						if (fromModule)
						{
							...
							/* Y */
							makeSimpleNimRefTag (name, NULL, K_UNKNOWN,
							                        PYTHON_UNKNOWN_INDIRECTLY_IMPORTED,
							                        XTAG_UNKNOWN);
							/* x.Y */
							if (isXtagEnabled (XTAG_QUALIFIED_TAGS))
							{
								...
								makeSimpleNimRefTag (name, fq, K_UNKNOWN,
								                        PYTHON_UNKNOWN_INDIRECTLY_IMPORTED,
								                        XTAG_QUALIFIED_TAGS);
								vStringDelete (fq);
							}
							/* Z */
							makeSimpleNimTag (token, K_UNKNOWN);
						}
						else
						{
							/* import x as Y
							 ...
							 * Y = (kind:namespace)*/
							/* X */
							makeSimpleNimRefTag (name, NULL, K_MODULE,
							                        PYTHON_MODULE_INDIRECTLY_IMPORTED,
							                        XTAG_UNKNOWN);
							/* Y */
							makeSimpleNimTag (token, K_NAMESPACE);
						}
...
static bool parseVariable (tokenInfo *const token, const nimKind kind)
{
	...
	if (token->type == '=')
	{
		...
		do
		{
			...
			if (! nameToken)
				...
				makeSimpleNimTag (nameToken, kind);
			else
			{
		...
		{
			if (nameTokens[i])
				makeSimpleNimTag (nameTokens[i], kind);
		}
	}
...
{
	NestingLevel *lv = nestingLevelsGetCurrent (NimNestingLevels);
	...
	while (lv && PY_NL (lv)->indentation >= token->indent)
	{
		...
		nestingLevelsPop (NimNestingLevels);
		lv = nestingLevelsGetCurrent (NimNestingLevels);
	}
}
...
static void findNimTags (void)
{
	...
	NextToken = NULL;
	NimNestingLevels = nestingLevelsNew (sizeof (struct nimNestingLevelUserData));
	...
	while (token->type != TOKEN_EOF)
	{
		...
		{
			nimKind kind = token->keyword == KEYWORD_class ? K_CLASS : K_FUNCTION;
		...
		else if (token->type == TOKEN_IDENTIFIER && atStatementStart)
		{
			...
			readNext = parseVariable (token, kind);
		         ...
		         NimFields[F_DECORATORS].enabled)
		{
			/* collect decorators */
			readQualifiedName (token);
			if (token->type != TOKEN_IDENTIFIER)
	...
	nestingLevelsFree (NimNestingLevels);
...
extern parserDefinition* NimParser (void)
{
	...
	static const char *const aliases[] = { "nim*", NULL };
	parserDefinition *def = parserNew ("Nim");
	def->kindTable = NimKinds;
	def->kindCount = ARRAY_SIZE (NimKinds);
	def->extensions = extensions;
	def->aliases = aliases;
	def->parser = findNimTags;
	def->initialize = initialize;
	def->finalize = finalize;
	def->keywordTable = NimKeywordTable;
	def->keywordCount = ARRAY_SIZE (NimKeywordTable);
	def->fieldTable = NimFields;
	def->fieldCount = ARRAY_SIZE (NimFields);
	def->useCork = true;
